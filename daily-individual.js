// daily-individual.js — ProAtletCare (PREMIUM COMPACT DONE)

(function() {
    const YOUTUBE_BASE = 'https://www.youtube.com/embed/';
    const STAGES_LIST = ['Pre-Training', 'Main Training', 'Post-Training'];

    // РЕКОМЕНДАЦІЇ ТЕПЕР ТУТ
                            ...
                            <div class="daily-exercise-item" style="background:#050505; border:1px solid #111; border-radius:12px; padding:20px; margin-bottom:12px; display:flex; gap:25px; align-items:center;">
                                ...
                                <div style="width:250px; text-align:right;">
                                    ...
                                    <div style="display:flex; justify-content:flex-end;">
                                        <label style="display:flex; align-items:center; gap:6px; background:#111; color:#d4af37; border:1px solid #333; padding:6px 12px; border-radius:20px; cursor:pointer; font-weight:900; font-size:0.6rem; text-transform:uppercase; transition:0.2s;">
                                            <input type="checkbox" style="width:14px; height:14px; accent-color:#d4af37;" onchange="this.closest('.daily-exercise-item').style.opacity = this.checked ? 0.2 : 1; this.parentElement.style.background = this.checked ? '#2ecc71' : '#111'; this.parentElement.style.color = this.checked ? '#000' : '#d4af37';">
                                            DONE
                                        </label>
                                    </div>
                                </div>
    ...
    // Решта функцій RPE без змін...
    function renderRpeForm() { /* як у попередньому варіанті */ }
