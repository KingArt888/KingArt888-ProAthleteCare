...
const WEEKLY_STORAGE_KEY = 'weeklyPlanData'; // Оновлено
const COLOR_MAP = {
...
function saveData(newWeeklyPlan = null, templatesFromUI = null) {
    ...
    try {
        let existingData = JSON.parse(localStorage.getItem(WEEKLY_STORAGE_KEY) || '{}'); // Оновлено
        const activityData = {};
        let finalPlanData = {};
        ...
        localStorage.setItem(WEEKLY_STORAGE_KEY, JSON.stringify(combinedData)); // Оновлено
...
function generateWeeklyPlan(mdStatuses, templates) {
    ...
    dayIndices.forEach(dayIndex => {
        ...
        const savedData = JSON.parse(localStorage.getItem(WEEKLY_STORAGE_KEY) || '{}'); // Оновлено
...
function loadData() {
    try {
        const savedData = localStorage.getItem(WEEKLY_STORAGE_KEY); // Оновлено
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modal = document.getElementById('exercise-selection-modal');
    if (modal) { // Додано перевірку, яка виправляє TypeError: null (reading 'addEventListener')
